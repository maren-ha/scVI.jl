...
-------------------------
 ...
 - `z_tsne`: a boolean variable to indicate whethe to futher encode the latent space to enable training with tsne loss that is integrated from scvis.
"""
...
- `var_eps`: numerical stability constant to add to the variance in the reparameterisation of the latent representation
- `z_tsne`: a boolean variable to indicate whethe to futher encode the latent space to enable training with tsne loss that is integrated from scvis.
- `tsne_components`: number of tsne dimensions, ´default=2´
"""
function scEncoder(
    n_input::Int, 
    n_output::Int;
